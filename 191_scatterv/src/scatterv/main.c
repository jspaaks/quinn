...
int main (int argc, char * argv[]) {
    ...
    // on rank 0 only, initialize the whole vector    
    if (irank == 0) {
        for (int ielem = 0; ielem < nelems.whole; ielem++) {
            vector.whole[ielem] = 100 + ielem;
        }       
    }
    ...
    // distribute the contents of vector.whole to each process's vector.chunk using MPI_Scatterv
    MPI_Scatterv(vector.whole, (const int *) lengths, (const int *) offsets, MPI_INT, vector.chunk, lengths[irank], MPI_INT, 0, MPI_COMM_WORLD);

    // sleep briefly to help print in order (not guaranteed)
    sleep(1);

    // let each process print its chunk once again
    print_vector(nelems.chunk, (const int *) vector.chunk, irank);
